...
<div id="mws-themer">
	<div id="mws-themer-content" style="<?php echo ($searchVariable) ? 'right: 256px;' : ''; ?>">
		...
		{{ Form::open(['method' => 'get','role' => 'form','url' => 'text-setting','class' => 'mws-form']) }}
		{{ Form::hidden('display') }}
		...
		<div class="mws-themer-section">
			...
			<a href="{{URL::to('text-setting')}}"  class="btn btn-default btn-small">Reset</a>
		</div>
		{{ Form::close() }}
	</div>
</div>
...
<div  class="mws-panel grid_8 mws-collapsible">
	<div class="mws-panel-header">
		...
		<a href="{{URL::to('text-setting/add-new-text')}}" class="btn btn-success btn-small align">{{ trans("messages.settings.add_new_setting") }}</a>
	</div>
				...
				@if(!$result->isEmpty())
					@foreach($result as $record)
						<tr>
							...
							<td data-th='Action'>
								<a href="{{URL::to('text-setting/edit-new-text/'.$record->id)}}" class="btn btn-info btn-small ml5"  >{{ trans("messages.settings.edit") }} </a>
							</td>
						</tr>
					@endforeach
					@else
