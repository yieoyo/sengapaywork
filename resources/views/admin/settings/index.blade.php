		...
		<table class="mws-table mws-datatable">
			...
			<tbody>
				...
				<tr>
					...
					<td data-th='Action'>
						<a href="{{URL::to('admin/settings/edit-setting/'.$record->id)}}" class="btn btn-info btn-small">{{ trans('Edit') }} </a>
						<a href="{{URL::to('admin/settings/delete-setting/'.$record->id)}}" data-delete="delete" class="btn btn-danger btn-small no-ajax"> {{{ trans('Delete') }}} </a>
					</td>
				</tr>
				 @endforeach  
				 @else
				<table class="mws-table mws-datatable details">	
					<tr>
						<td align="center" width="100%"> {{{ trans('No Records Found') }}}</td>
					</tr>	
				</table>
				@endif 
			</tbody>
