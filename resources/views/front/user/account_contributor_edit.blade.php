				...
				<div class="kt-subheader__main">
					<h3 class="kt-subheader__title">
						<button class="kt-subheader__mobile-toggle kt-subheader__mobile-toggle--left" id="kt_subheader_mobile_toggle"><span></span></button>
							{{ trans("messages.account_contributors.edit_contributors") }}</h3>
					<div class="kt-subheader__breadcrumbs">
						...
						<span class="kt-subheader__breadcrumbs-separator"></span>
						<a href="{{route('user.account_contributor')}}" class="kt-subheader__breadcrumbs-link">
							{{ trans("messages.account_contributors.contributors") }}  </a>
						<span class="kt-subheader__breadcrumbs-separator"></span>
						<span class="kt-subheader__breadcrumbs-link">
							 {{ trans("messages.account_contributors.edit_contributors") }} </span>
					</div>
				</div>
			</div>
		</div>
				...
				<div class="kt-portlet">
					<div class="kt-portlet__head">
						<div class="kt-portlet__head-label">
							<h3 class="kt-portlet__head-title"> {{ trans("messages.account_contributors.edit_contributor_form") }}</h3>
						</div>
					</div>
					{{ Form::open(['role' => 'form','url'=>"contributor-save", 'class'=>'kt-form kt-form--label-left', 'id'=>"editSalesPersonForm"]) }}
						...
						<div class="row">
							<div class="col-sm-6">
								<div class="form-group cot_form_input_holder">
									...
									{{ Form::text('full_name',$userDetails->full_name,['class'=>'form-control','autocomplete'=>'off', 'placeholder'=>trans('messages.personal_information.enter_full_name')]) }}
							...
							<div class="col-sm-6">
								<div class="form-group cot_form_input_holder">
									...
									<div class="input-group">
										...
										{{ Form::text('email',$userDetails->email,['class'=>'form-control','autocomplete'=>'off', 'placeholder'=>trans(''), 'aria-describedby'=>'basic-addon1']) }}
									</div>
							...
							<div class="col-sm-6">
								<div class="form-group cot_form_input_holder">
									...
									{{ Form::select('is_active',array('1'=>trans('messages.navigation.active'),'0'=>trans('messages.language_settings.in_active')),$userDetails->is_active, ['class'=>'form-control', 'autocomplete'=>'off', 'placeholder'=>trans('trans("messages.admin_account.select_status")')]) }}
					  ...
					  <div class="kt-form__actions">
						<div class="row">
						  <div class="col-lg-9 col-xl-9">
							...
							<a href="{{ route('user.account_contributor') }}" class="btn btn-secondary">{{ trans('messages.personal_information.cancel') }}</a>
						  </div>
						</div>
					  </div>
					</div>
...
function saveSalesPerson(KeyVal){
	...
	$.ajax({
		url: '{{ route("User.AccountContributorUpdate") }}',
		type:'POST',
		...
		success: function(r){
			...
			if(datas['success'] == 1) {
				if(KeyVal == 2){
					//location.reload();
					window.location.href	 =	"{{ route('user.account_contributor_add') }}";
				}else{
					window.location.href	 =	"{{ route('user.account_contributor') }}";
				}
			}else {
