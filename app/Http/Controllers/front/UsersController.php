...
class UsersController extends BaseController
{
	...
	public function InvoiceGenerate($invoiceId = "")
	{
		...
		if (!empty($paymentInvoice)) {
			...
			$orderDetails = DonationOrder::LeftJoin('sub_projects', 'donation_orders.sub_project_id', '=', 'sub_projects.id')
				->select(
					'donation_orders.*',
					'sub_projects.sub_project_name as sub_project_name',
					'sub_projects.payment_type as payment_type',
	...
	public function PayNow()
	{
		...
		if (!empty(Input::get('order_id'))) {
			...
			$subProjectDetails = SubProject::select('id', 'project_id', 'sub_project_name', 'project_module', 'customize_plan_option', 'slug')->where('id', $orderDetails->sub_project_id)->first();
	...
	public function AdminDashboard()
	{
		...
		//recent infaq
		$recentAllOrders = DonationOrder::select(
			'donation_orders.*',
		...
		$recentSuccessOrders = DonationOrder::select(
			'donation_orders.*',
			DB::raw("(select slug from sub_projects where id=donation_orders.sub_project_id) as sub_project_slug"),
			DB::raw("(select sub_project_name from sub_projects where id=donation_orders.sub_project_id) as sub_project_name"),
			DB::raw("(select project_module from sub_projects where id=donation_orders.sub_project_id) as project_module")
		...
		$recentPendingOrders = DonationOrder::select(
			'donation_orders.*',
			DB::raw("(select slug from sub_projects where id=donation_orders.sub_project_id) as sub_project_slug"),
			DB::raw("(select sub_project_name from sub_projects where id=donation_orders.sub_project_id) as sub_project_name"),
			DB::raw("(select project_module from sub_projects where id=donation_orders.sub_project_id) as project_module")
		...
		$recentWaitingOrders = DonationOrder::select(
			'donation_orders.*',
			DB::raw("(select slug from sub_projects where id=donation_orders.sub_project_id) as sub_project_slug"),
			DB::raw("(select sub_project_name from sub_projects where id=donation_orders.sub_project_id) as sub_project_name"),
			DB::raw("(select project_module from sub_projects where id=donation_orders.sub_project_id) as project_module")
		...
		if (!empty($seatReservationPlans)) {
			foreach ($seatReservationPlans as &$seatReservationPlan) {
				$seatReservationDetails = DB::table('sub_project_seat_reservation_plans')
					->select(
						'sub_project_seat_reservation_plans.*',
	...
	{	
		...
		if ($validator->fails()) {
			...
			$order_id = urldecode($model->id);
			$name = urldecode($model->full_name);
			$email = urldecode($model->email);
			$phone = urldecode($model->phone);
